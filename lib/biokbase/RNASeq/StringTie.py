...
class StringTie(ExecutionBase): 
    ...
    def runEach(self,task_params):
        ...
        try:
                ...
                input_direc = os.path.join(directory,read_sample.split('.')[0]+"_stringtie_input")
                if not os.path.exists(input_direc): os.mkdir(input_direc)
                output_name = read_sample.split('.')[0]+"_stringtie_alignment"
                output_dir = os.path.join(directory,output_name)
                if not os.path.exists(output_dir): os.mkdir(output_dir)
                print directory
                base = handler_util.get_file_with_suffix(directory,".1.ht2")
                print base
                stringtie_base =os.path.join(directory,base)
                ### Adding advanced options to Bowtie2Call
                stringtie_cmd = ''
                stringtie_cmd += ( ' -p {0}'.format(self.num_threads))
                if('quality_score' in params and params['quality_score'] is not None): stringtie_cmd += ( ' --'+params['quality_score'])
                if('alignment_type' in params and params['alignment_type'] is not None): stringtie_cmd += ( ' --'+params['alignment_type'] )
                if('trim5' in params and params['trim5'] is not None): stringtie_cmd += ( ' --trim5 '+str(params['trim5']))
                if('trim3' in params and params['trim3'] is not None): stringtie_cmd += ( ' --trim3 '+str(params['trim3']))
                if('np' in params and params['np'] is not None): stringtie_cmd += ( ' --np '+str(params['np']))
                if('minins' in params and params['minins'] is not None): stringtie_cmd += ( ' --minins '+str(params['minins']))
                if('maxins' in params and params['maxins'] is not None): stringtie_cmd += ( ' --maxins '+str(params['maxins']))
                #if('orientation' in params and params['orientation'] is not None): stringtie_cmd += ( ' --'+params['orientation'])
                if('min_intron_length' in params and params['min_intron_length'] is not None): stringtie_cmd += ( ' --min-intronlen '+str(params['min_intron_length']))
                if('max_intron_length' in params and params['max_intron_length'] is not None): stringtie_cmd += ( ' --max-intronlen '+str(params['max_intron_length']))
                if('no_spliced_alignment' in params and params['no_spliced_alignment'] != 0): stringtie_cmd += ( ' --no-spliced-alignment')
                if('transcriptome_mapping_only' in params and params['transcriptome_mapping_only'] != 0): stringtie_cmd += ( ' --transcriptome-mapping-only')
                if('tailor_alignments' in params and params['tailor_alignments'] is not None): 
                    stringtie_cmd += ( ' --'+params['tailor_alignments'])
                ...
                if sample_type  == 'KBaseAssembly.SingleEndLibrary':
                        ...
                        try:
                                print read_id
                                print read_name
                                print self.urls['shock_service_url']
                                print input_direc
                                ...
                                stringtie_cmd += " -U {0} -x {1} -S {2}".format(os.path.join(input_direc,read_name),stringtie_base,out_file)
                        except Exception,e:
                ...
                if sample_type == 'KBaseAssembly.PairedEndLibrary':
                        ...
                        try:
                                ...
                                stringtie_cmd += " -1 {0} -2 {1} -x {2} -S {3}".format(os.path.join(input_direc,read1_name),os.path.join(output_dir,read2_name),stringtie_base,out_file)
                        except Exception,e:
                ...
                try:
                        self.logger.info("Executing: stringtie {0}".format(stringtie_cmd))
                        cmdline_output = script_util.runProgram(self.logger,"stringtie",stringtie_cmd,None,directory)
                except Exception,e:
                        raise Exception("Failed to run command {0}".format(stringtie_cmd))
                        #logger.exception("Failed to run command {0}".format(stringtie_cmd))
                try:
                        stats_data = {}
                        ...
                        raise Exception("Error Running the stringtie command {0},{1} {2}".format(stringtie_cmd,directory," ".join(traceback.print_exc())))
                        #logger.exception("Error Running the stringtie command {0},{1} {2}".format(stringtie_cmd,directory," ".join(traceback.print_exc())))
                ...
                try:
                        stringtie_handle = hs.upload(out_file_path)
                except Exception, e:
		...
		logger.info("Preparing output object")
                stringtie_out = { "file" : stringtie_handle ,"size" : os.path.getsize(out_file_path), "aligned_using" : "stringtie" , "aligner_version" : "2.2.6" , 'library_type' : lib_type , 'condition' : condition ,'read_sample_id': read_sample, 'genome_id' : genome_id , "alignment_stats" : stats_data }
                if not sampleset_id is None: stringtie_out['sampleset_id'] = sampleset_id
                pprint(stringtie_out)
		try:
			...
			raise Exception(e)
                        ...
                        #logger.exception("Failed to create stringtie Alignment {0}".format(" ".join(traceback.print_exc())))
                        raise Exception("Failed to create stringtie Alignment {0}".format(" ".join(traceback.print_exc())))
        finally:
