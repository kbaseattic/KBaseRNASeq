#!/usr/bin/python
# *****************************************************************************
# call_utils.py (Tophat , Cufflinks , Cuffmerge , Cuffdiff calls defined)
#******************************************************************************
...
class BaseCall(object):
    ...
    def __init__(self, yargs, run_id, run_logs,conditions,mode='analyze'):
        """
...
class TophatCall(BaseCall):
    ...
    def __init__(self, yargs, run_id, run_logs, conditions,mode):
        """
        ...
        :param run_id: id for the whole set of calls
        :param run_logs: the directory where log file should be put
	:param conditions: one or a list of condition-dictionaries from ``yargs.conditi|    else:
	on_queue``	
        :param mode: choices = ['analyze','dry_run','qsub_script']
        ...
        BaseCall.__init__(self, yargs, run_id, run_logs,conditions,mode)

        self.prog_yargs = self.yargs.tophat_options
	self.positional_yargs = self.prog_yargs.positional_args
        self.set_call_id()
        self.init_log_file()
	...
	if "G" in self.prog_yargs:
        	self.opt_dict['G'] = self.get_gtf_anno()
        self.construct_options_list()

        # now the positional args
        bowtie_index = self.get_bt_idx()
	#print 'yargs in call_utils' + json.dumps(yargs)
	print 'prog_yargs ' + json.dumps(self.prog_yargs)
	if 'singleend_reads' in self.positional_yargs.keys():
		print 'in if loop'
		singleend_reads = self.get_singleend_reads()
       		self.options_list.extend([bowtie_index, singleend_reads])
	else:		 
		print  'in else loop'
    ...
    def get_singleend_reads(self):
        """
        Handles ``yaml_config.tophat_options.positional_args.singleend_reads: from_conditions``.
        """
